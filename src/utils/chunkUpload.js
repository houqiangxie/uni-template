/**
 * 大文件分片上传（H5 / 微信小程序 / APP 统一实现）
 * - 跨平台文件系统
 * - 本地持久化（队列 + 文件本体）
 * - 分片读取与上传、断点续传、失败重试、暂停/继续
 */
...
// ---------------------------------------------------------------------------
// 跨平台文件系统
// uni.getFileSystemManager() 仅鸿蒙 APP 支持
// ---------------------------------------------------------------------------
...
function splitPath(filePath) {
  ...
  return {
    dir: idx >= 0 ? normalized.slice(0, idx) : '',
    name: idx >= 0 ? normalized.slice(idx + 1) : normalized,
  }
}

function createAppPlusFileSystemManager() {
  return {
    readFile({ filePath, position, length, success, fail }) {
      plus.io.resolveLocalFileSystemURL(toLocalUrl(filePath), (entry) => {
        entry.file((file) => {
          const target = position != null && length != null
            ? file.slice(position, position + length)
            : file
          const reader = new plus.io.FileReader()
          reader.onloadend = (evt) => {
            success?.({ data: evt.target?.result })
          }
          reader.onerror = (err) => {
            fail?.(err)
          }
    ...
    writeFile({ filePath, data, success, fail }) {
      const { dir, name } = splitPath(toLocalUrl(filePath))
      const payload = data instanceof ArrayBuffer ? new Blob([data]) : data
      plus.io.resolveLocalFileSystemURL(dir, (dirEntry) => {
        dirEntry.getFile(name, { create: true }, (fileEntry) => {
          fileEntry.createWriter((writer) => {
            writer.onwrite = () => success?.({})
            writer.onerror = err => fail?.(err)
    ...
    copyFile({ srcPath, destPath, success, fail }) {
      const { dir, name } = splitPath(toLocalUrl(destPath))
      plus.io.resolveLocalFileSystemURL(toLocalUrl(srcPath), (srcEntry) => {
        plus.io.resolveLocalFileSystemURL(dir, (dirEntry) => {
          srcEntry.copyTo(dirEntry, name, () => success?.({}), fail)
        }, fail)
      }, fail)
    },

    access({ path, success, fail }) {
      plus.io.resolveLocalFileSystemURL(toLocalUrl(path), () => success?.({}), fail)
    },
...
function getFileSystemManager() {
  ...
  // #endif
  // #ifdef MP-QQ
  return qq.getFileSystemManager()
  // #endif
  // #ifdef APP-HARMONY
  return uni.getFileSystemManager()
  // #endif
  // #ifdef APP-PLUS
  return createAppPlusFileSystemManager()
  // #endif
}

// ---------------------------------------------------------------------------
// 本地持久化（队列 + 文件本体）
// H5：IndexedDB 存 Blob；小程序 / App：复制到用户目录
// ---------------------------------------------------------------------------
...
export function saveQueueSession(sessionId, items) {
  try {
    ...
    console.warn('[chunkUpload] save queue failed', e)
  }
}
...
export function removeQueueSession(sessionId) {
  try {
    ...
    console.warn('[chunkUpload] remove queue failed', e)
  }
}
...
export async function persistFile(uid, raw) {
  ...
  try {
    ...
    console.warn('[chunkUpload] H5 persist failed', e)
    throw e
  }
  // #endif
...
export async function hydrateFileItem(item) {
  ...
  if (item.storeType === 'idb' || (!item.storeType && item.persistPath && item.persistPath === item.uid)) {
    ...
    try {
      ...
      console.warn('[chunkUpload] hydrate idb failed', e)
    }
  }
  // #endif
...
export async function removePersistedFile(item) {
  ...
  if (item.storeType === 'idb') {
    try {
      ...
      console.warn('[chunkUpload] remove idb failed', e)
    }
